...
var secukitnxInfo = {
    ...
    "moduleInfo": {
        ...
        "exNxClient_eds_dll":Module_EDS_Ver
    },
    "exServiceInfo": {
        "isUse": true,
...
var SECUKITNX_CONST = {
    ...
    "cmdCnt": [],
    "intervalCheckCnt": 0,
    "serviceType": 2,                       // XML & KMS 이용시  : nx 1,              TAX(xml) 2,              kms 3
        ...
        if (currPluginCnt >= SECUKITNX_CONST.pluginCount) {
            ...
            for (var i = 0; i < SECUKITNX_CONST.pluginCount; i++) {
                ...
                if (!SECUKITNX_CONST.multiOS) {
                    ...
                    if (SECUKITNX_CONST.serviceType === 4) {
                        if (!currStatus.eds) {
                            chk = false;
                            currInstalled = false;
                        }
            ...
            if (!SECUKITNX_CONST.multiOS) {
                ...
                if (updateInfo.reply.result && updateInfo.reply.result.kpmsvc) {
                    try {
                        ...
                        // check EDS module version
                        if (SECUKITNX_CONST.serviceType === 4) {
                            tmpINSTALLVer = pluginInfo.moduleInfo.exNxClient_eds_dll;
                            // json response value에 이름을 맞춤 secukitEDS
                            if (updateInfo.reply.result.secukitEDS) {
                                if (SECUKITNX_UTIL.diffVersion(updateInfo.reply.result.secukitEDS, tmpINSTALLVer) === false) {
                                    SECUKITNX_CHECK.setServiceStatus("eds", updateInfo.reply.result.secukitEDS, false, true);
                                }
                                else {
                                    SECUKITNX_CHECK.setServiceStatus("eds", updateInfo.reply.result.secukitEDS, true, true);
                                }
                            }
                            else {
                                SECUKITNX_CHECK.setServiceStatus("eds", "", false, true);
                            }
                        }
    ...
    setServiceStatus: function (type, localVer, status, isNext) {
        ...
        switch(type)
        {
            ...
            case 'xml':
                ...
                break;
            case 'eds':
                SECUKITNX_CHECK.chkInfoStatus.info[SECUKITNX_CHECK.chkCurrPluginCnt].edsVer = localVer;
                SECUKITNX_CHECK.chkInfoStatus.info[SECUKITNX_CHECK.chkCurrPluginCnt].eds = status;
                break;
        }
    }
};
        ...
        if (SECUKITNX_CONST.nextVerCheck == true)
        {
            switch(SECUKITNX_CONST.serviceType)
            {
                ...
                case 3: errorcode = 'ERR_CLIENT_RELEASE_VERSION_KMS';
                    break;
                case 4: errorcode = 'ERR_CLIENT_RELEASE_VERSION_EDS';
            		break;
            ...
            switch (SECUKITNX_CONST.serviceType) {
                ...
                case 3: errorcode = 'ERR_CLIENT_NO_INSTALL_KMS';
                    break;
                case 4: errorcode = 'ERR_CLIENT_NO_INSTALL_EDS';
            		break;
