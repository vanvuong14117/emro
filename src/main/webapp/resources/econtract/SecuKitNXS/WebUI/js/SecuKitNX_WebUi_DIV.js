...
KICA_SECUKITNXDIV += '<h1>' + NX_WEBUI_SELECT_TEXT_2 + ' <a href="javascript:void(0)" class="btn-setting" onclick="NX_Issue_pubUi.managemnetPageOpen(); NXClientGetVersion();">' + NX_WEBUI_SELECT_TEXT_1 + '</a> </h1>';
...
KICA_SECUKITNXDIV += '<div class=\"pki-head3\"><h1>' + NX_WEBUI_SELECT_TEXT_13 + '</h1><a href=\"javascript:void(0);\" class=\"btn-close1\" onclick=\"NX_Issue_pubUi.moreSaveMediaHide5();return false;\">닫기</a></div>';
KICA_SECUKITNXDIV += '<div class=\"pki-bottom3\">';
KICA_SECUKITNXDIV += '<div class=\"gray-box2 mb10\"> ' + NX_WEBUI_SELECT_TEXT_14 + ' </div>';
KICA_SECUKITNXDIV += '<input id="nx_cert_pin" name="nx_cert_pin"  type="password" size=20 onkeydown="NX_Issue_pubUi.EnterOnPINNumber(event)" title="보안토큰 비밀번호">';
...
if (NOS_F === true) {
    //잉카 키보드보안 연동을 위한 수정
    KICA_SECUKITNXDIV += '<input tabindex="0" type=\"password\" id=\"certPwd\" name=\"RePwName\" npkencrypt=\"Sub\" style=\"width: 100%;\" title=\"인증서 암호\" />';
}
else if (RAON_F === true) {
    //라온시큐어 키보드보안 연동을 위한 수정
    KICA_SECUKITNXDIV += '<input tabindex="0" type=\"password\" id=\"certPwd\" name=\"certPwd\" style=\"width: 100%;\"  data-enc=\"on\" title=\"인증서 암호\" />';
}
else {
    KICA_SECUKITNXDIV += '<input tabindex="0" type=\"password\" id=\"certPwd\" style=\"width: 100%;\" title=\"인증서 암호\" />';
}
...
KICA_SECUKITNXDIV += '</div>';
KICA_SECUKITNXDIV += '<div id=\"certContent_03\" class=\"cert-content\" style="display: none;">';
